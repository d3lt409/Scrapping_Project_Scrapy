...
from scraper.spiders.constants import jumbocl
from scrapy.http import Response
from playwright.async_api import Page


class jumboclclSpider(scrapy.Spider):
    name = "jumboclcl"
    pais = "chile"
    allowed_domains = ["www.jumbocl.cl"]
    start_urls = ["https://www.jumbocl.cl/"]
    ...
    def __init__(self, name=None, **kwargs):
        ...
        self.start_urls = []
        if category:
            if category in 'supermarket' and part and part == '1':
                self.start_urls = jumbocl.START_URLS_SUPERMARKET_1
            elif category in 'supermarket' and part and part == '2':
                self.start_urls = jumbocl.START_URLS_SUPERMARKET_2
            elif category in 'electronic':
                self.start_urls = jumbocl.START_URLS_ELECTRO
        else:
            self.start_urls = jumbocl.START_URLS_SUPERMARKET_1 + \
                jumbocl.START_URLS_SUPERMARKET_2 + jumbocl.START_URLS_ELECTRO

    async def start(self):
        self.logger.info("Lanzando navegador Playwright...")
        for url in self.start_urls:
            self.logger.info(
                f"Iniciando scraping para la categoría principal: {url}")
            yield scrapy.Request(
                url,
                meta=dict(playwright=True, playwright_include_page=True,
                          playwright_page_methods=[
                              PageMethod(
                                  "wait_for_selector", jumbocl.SELECTOR_LOAD_PRODUCTS, timeout=90000),
                          ],
        ...
        self.logger.info("Todas las solicitudes iniciales han sido enviadas.")

    async def handle_age_verification(self, page: Page):
        try:
            await page.wait_for_selector(jumbocl.SELECTOR_OLDER_AGE, timeout=10000)
            original_url = page.url
            self.logger.info(
                "Página de verificación de edad detectada. Intentando hacer clic en el botón...")
            older_age_button = page.locator(jumbocl.SELECTOR_OLDER_AGE)
            if await older_age_button.count() > 0:
                await older_age_button.first.click()
                self.logger.info(
                    "Clic en el botón de verificación de edad realizado.")
                # Espera para asegurarse de que la página se actualice
                await page.wait_for_timeout(2000)
                await page.goto(original_url, wait_until="domcontentloaded", timeout=60000)
            else:
                self.logger.info(
                    "No se encontró el botón de verificación de edad.")
        except Exception as e:
            self.logger.info(
                f"No se detectó una página de verificación de edad. Continuando con el scraping. Error: {e}")
    ...
    async def orchestrator(self, response: Response):
        ...
        try:
            ...
            total_products_element = await page.wait_for_selector(jumbocl.SELECTOR_TOTAL_COUNT_PRODUCTS, timeout=15000)
            total_products_text = await total_products_element.inner_text()
            total_products = int(re.sub(r'[^\d]', '', total_products_text))
        except Exception as e:
            self.logger.warning(
                f"No se pudo determinar el total de productos para {response.url}. Procediendo con scrapeo directo. Error: {e}")
            await page.wait_for_timeout(2000)
            total_products = 0
        try:
            self.logger.info(
                f"Total de productos en {response.url}: {total_products}")
            if total_products > 1400:
                self.logger.info(
                    f"La categoría {response.url} tiene {total_products} productos. Se requiere descubrir subcategorías.")
                async for request in self.discover_subcategories(response):
                    yield request
            else:
                self.logger.info(
                    f"La categoría {response.url} tiene {total_products} productos. Scrapeando directamente.")
                async for item in self.parse_category(response):
                    yield item
        except Exception as e:
            self.logger.error(
                f"Error durante la orquestación para {response.url}: {e}")
            await page.close()
            ...
            while True:
                try:
                    ...
                    next_page_button = page.locator(
                        f"xpath={jumbocl.XPATH_CLICK_BUTTON}")

                    if await next_page_button.count() > 0:
                        self.logger.info(
                            "Botón 'Siguiente' encontrado. Navegando a la siguiente página...")
                        await page.wait_for_timeout(2000)
                        await next_page_button.first.click()
                        self.logger.info(
                            "Clic en el botón 'Siguiente' realizado.")
                        await page.wait_for_timeout(2000)
                        await page.wait_for_selector(jumbocl.SELECTOR_LOAD_PRODUCTS, timeout=90000)
                        page_number += 1
                    else:
                        self.logger.info(
                            "No se encontró el botón 'Siguiente'. Fin de la categoría.")
                        break
                except Exception as e:
                    self.logger.error(
                        f"Error al procesar la página {page_number} de {response.url}: {e}")
                    break
        except Exception as e:
            self.logger.error(
                f"Error al parsear la categoría {response.url} en la página {page_number}: {e}")
        finally:
            await page.close()
            self.logger.info(f"Cerrada la página para {response.url}")

    async def discover_subcategories(self, response: Response):
        page: Page = response.meta["playwright_page"]
        try:
            self.logger.info("Etapa 1: Descubriendo subcategorías...")
            await page.click(jumbocl.XPATH_BUTTON_GET_CATEGORIES)
            await page.wait_for_timeout(2000)
            pages = page.url.split("/")
            selector_get_category = f"a[href='/{pages[-2].strip()}'][class*='jumbocl-main-menu-2-x-link--header-submenu-item']"
            ...
            get_sub_category = f"{jumbocl.XPATH_CATEGORY_SUB_CATEGORY}/div/span/a[contains(@href,'/{pages[-1].strip()}')]/../..//a[contains(@class, 'item_node_inner_third_level--header-submenu-item')]"
            ...
            for link in links:
                ...
                self.logger.info(
                    f"Se encontraron {len(subcategory_urls)} URLs de subcategorías.")
        except Exception as e:
            self.logger.error(
                f"Error al descubrir subcategorías en {response.url}: {e}")
        finally:
            await page.close()
    ...
    async def save_products_count(self, page, page_number, response):
        try:
            self.logger.info(
                f"Scrapeando página {page_number} de '{response.url}'...")
            ...
            breadcrumbs = scrapy_selector.xpath(
                jumbocl.XPATH_GET_BREADCRUMBS).getall()
            cat_sub = response.url.replace(
                "https://www.jumboclcolombia.com/", "").split('/')
            if len(breadcrumbs) == 1:
                breadcrumbs = [breadcrumbs[0].replace('-', ' ').title(),
                               breadcrumbs[0].replace('-', ' ').title()]
            elif len(breadcrumbs) < 2 and len(cat_sub) >= 2:
                breadcrumbs = [cat_sub[0].replace('-', ' ').title(),
                               cat_sub[1].replace('-', ' ').title()]
            elif len(cat_sub) == 1 and len(breadcrumbs) < 1:
                breadcrumbs = [cat_sub[0].replace('-', ' ').title(),
                               cat_sub[0].replace('-', ' ').title()]

            category = breadcrumbs[0]
            sub_category = breadcrumbs[1]
            for product_card in scrapy_selector.xpath(jumbocl.XPATH_GET_ALL_PRODUCTS):
                yield self.take_products_fields(product_card, category, sub_category)
        except Exception as e:
            self.logger.error(
                f"Error al guardar productos en la página {page_number} de {response.url}: {e}")
            self.logger.error(
                f"Producto no guardado con breadcrumbs: {breadcrumbs}")
    ...
    def take_products_fields(self, product_card: scrapy.Selector, category, sub_category):
        ...
        try:
            ...
            if len(matches) > 0:
                ...
                matches = re.findall(
                    jumbocl.REGULAR_EXPRESSION_UNITS_SIMPLE, name.lower(), re.IGNORECASE)
                if len(matches) > 0:
                    value = matches[-1]
            ...
            item['comercial_name'] = jumbocl.NAME

            item['comercial_id'] = jumbocl.ID

            price_text = product_card.xpath(jumbocl.XPATH_GET_PRICE).getall()
            if len(price_text) == 0:
                raise ValueError(
                    "Precio no encontrado, continúo al siguiente producto.")
            item['price'] = float(price_text[-1].strip().replace(
                '$', '').replace('.', '').replace(",", ".").strip())

            unit_price_text = product_card.xpath(
                jumbocl.XPATH_UNIT_PRICE).get()
            if unit_price_text:
                # 1. Busca el patrón de número (puede tener comas o puntos) en el texto
                price_match = re.search(
                    r'(\d+[\.,]\d*)', unit_price_text.strip())
                if price_match:
            ...
            self.logger.error(
                f"Error al extraer campos del producto {item.__dict__}: {e}")
            return item

    async def await_products_loaded(self, page: Page):
        # Espera inicial para asegurarse de que la página esté completamente cargada
        await page.wait_for_timeout(2000)
        await page.wait_for_selector(jumbocl.SELECTOR_LOAD_PRODUCTS, timeout=60000)
        previous_height = -1
        while True:
            try:
                await page.wait_for_selector(jumbocl.SELECTOR_CONTAINER_PRODUCTS, timeout=60000)
            except Exception:
                self.logger.warning(
                    "El contenedor de productos no apareció. Saltando el scroll.")
                break
            current_height = await page.evaluate(f"document.querySelector('{jumbocl.SELECTOR_CONTAINER_PRODUCTS}').scrollHeight")
