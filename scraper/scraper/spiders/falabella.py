...
from scraper.spiders.constants import falabella
...
class FalabellaSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.falabella.com/falabella-cl"]
                    ...
                    yield scrapy.Request(
                        ...
                        meta=dict(playwright=True, playwright_include_page=True,
                                  playwright_page_methods=[
                                      PageMethod(
                                          "wait_for_selector", falabella.SELECTOR_LOAD_PRODUCTS, timeout=90000),
                                  ],
                                  playwright_page_goto_kwargs={
                                      "wait_until": "domcontentloaded",
                                      "timeout": 60000
    ...
    async def _get_categories(self, page: Page):
        ...
        button_categories = await page.wait_for_selector(
            falabella.SELECTOR_BUTTON_CATEGORIES)
        ...
        categories_urls = {}
        if self.category:
            categories_elements = page.locator(falabella.XPATH_CATEGORY_NAME.replace(
            ...
            categories_elements = await page.locator(
                falabella.XPATH_CATEGORIES).all()
            ...
            for element in categories_elements:
                ...
                if category_name in falabella.LIST_CATEGORIES:
    ...
    async def _get_sub_categories(self, page: Page, category_name: str, categories_urls: dict):
        ...
        sub_categories_elements = await page.query_selector_all(
            falabella.SELECTOR_SUB_CATEGORIES)

        for sub_element in sub_categories_elements:
            # Obtenemos el nombre de la subcategoría dentro del elemento actual
            sub_category_name = await sub_element.query_selector(falabella.SELECTOR_SUB_CATEGORY_NAME)
            if sub_category_name:
                ...
                links = await sub_element.query_selector_all(
                    falabella.SELECTOR_SUB_CATEGORIES_LINKS)
                url_links = []
    ...
    async def parse(self, response: Response, category, sub_category):
        ...
        while True:
            ...
            try:
                await page.wait_for_timeout(2000)
                await page.wait_for_selector(falabella.SELECTOR_PRODUCT_CARDS)
                await page.wait_for_timeout(2000)
            ...
            try:
                await page.wait_for_selector(falabella.SELECTOR_PRODUCT_NAME, timeout=60000)
                await page.wait_for_timeout(2000)
                html_content = await page.content()
                scrapy_selector = scrapy.Selector(text=html_content)

                products = scrapy_selector.xpath(
                    falabella.XPATH_PRODUCT_CARDS)
                for product_card in products:
                    ...
                    item['comercial_name'] = falabella.NAME
                    item['comercial_id'] = falabella.ID
                    yield item
            ...
            try:
                next_page_button: Locator = page.locator(
                    falabella.XPATH_NEXT_PAGE_BUTTON)
    ...
    def _get_price(self, product_card):
        price = product_card.xpath(falabella.XPATH_PRODUCT_PRICE)
        if price:
            return float(price.get().replace(
                '$', '').replace('.', '').strip())

        price = product_card.xpath(falabella.XPATH_PRODUCT_PRICE1)
        if (price):
            return float(price.get().replace(
                '$', '').replace('.', '').strip())
        price = product_card.xpath(falabella.XPATH_PTODUCT_PRICE2)
        if (price):
            return float(price.get().replace(
                '$', '').replace('.', '').strip())
        else:
    ...
    def take_products_fields(self, product_card: scrapy.Selector, category, sub_category):
        ...
        try:
            name = product_card.xpath(
                falabella.XPATH_PRODUCT_NAME).get().strip()
        except Exception as e:
            raise ValueError(
    ...
    async def _await_products_loaded(self, page: Page):
        ...
        while True:
            try:
                await page.wait_for_selector(falabella.SELECTOR_CONTAINER_PRODUCTS, timeout=60000)
            except Exception:
                self.logger.warning(
                    "El contenedor de productos no apareció. Saltando el scroll.")
                break
            current_height = await page.evaluate(f"document.querySelector('{falabella.SELECTOR_CONTAINER_PRODUCTS}').scrollHeight")
