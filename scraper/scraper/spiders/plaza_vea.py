...
class PlazaVeaSpider(scrapy.Spider):
    ...
    def _process_input_urls(self, custom_urls):
        if custom_urls:
            urls = custom_urls.split(',') if isinstance(
                custom_urls, str) else custom_urls
            cleaned_urls = [url.strip() for url in urls]
            self.logger.info(
                f"Usando URLs personalizadas: {len(cleaned_urls)} URLs")
            return cleaned_urls
        else:
            category = self.category if hasattr(self, 'category') else None
            if category and category in "mercado":
                ...
                if part and part == '1':
                    urls = [
                        ...
                        for categoria, subcategorias in plaza_vea.CATEGORIAS_MERCADO1.items()
                        for subcategoria in subcategorias
                    ]
                    self.logger.info(
                        f"URLs generadas para 'mercado' parte 1: {len(urls)} URLs")
                    return urls
                    ...
                    urls = [
                        ...
                        for categoria, subcategorias in plaza_vea.CATEGORIAS_MERCADO2.items()
                        for subcategoria in subcategorias
                    ]
                    self.logger.info(
                        f"URLs generadas para 'mercado' parte 2: {len(urls)} URLs")
                    return urls
            elif category and category in "electronica":
                urls = [
                    plaza_vea.BASE_URL + plaza_vea.CATEGORIA_URL_TEMPLATE.format(
                        categoria=categoria, subcategoria=subcategoria
                    )
                    for categoria, subcategorias in plaza_vea.CATEGORIA_ELECTRONICA.items()
                    for subcategoria in subcategorias
                ]
                self.logger.info(
                    f"URLs generadas para categoría '{category}': {len(urls)} URLs")
                return urls
            ...
            yield scrapy.Request(
                ...
                meta=dict(
                    ...
                    playwright_page_methods=[
                        PageMethod(
                            "wait_for_selector", plaza_vea.SELECTOR_PRODUCTS_CONTAINER, timeout=30000),
                        PageMethod(
    ...
    async def parse_category(self, response: Response):
        ...
        self.logger.info(
            f"INICIANDO Request #{category_index} para: {categoria} > {subcategoria}")

        if page.url != original_url:
            self.logger.info(
                f"Navegando explícitamente a URL original: {original_url}")
            try:
                await page.goto(original_url, wait_until="domcontentloaded", timeout=30000)
                # Mayor tiempo de espera después de navegar
        ...
        while page_number <= max_pages:
            self.logger.info(
                f"Scrapeando página {page_number} de '{categoria} > {subcategoria}'...")
            ...
            self.logger.info(
                f"Elementos de producto encontrados: {len(products_elements)}")

            for product_element in products_elements:
                item = self.extract_product_data(
                    product_element, categoria, subcategoria)
                if item:
                    products_found += 1
            ...
            self.logger.info(
                f"Productos válidos extraídos en página {page_number}: {products_found}")
            self.logger.info(
                f"Total acumulado para '{categoria} > {subcategoria}': {total_products_scraped}")

            # Manejar páginas vacías o sin productos válidos
            if products_found == 0:
                consecutive_empty_pages += 1
                self.logger.warning(
                    f"Página {page_number} sin productos válidos. Páginas vacías consecutivas: {consecutive_empty_pages}/{max_consecutive_empty}")

                # Si hay demasiadas páginas vacías consecutivas, terminar esta categoría
                if consecutive_empty_pages >= max_consecutive_empty:
                    self.logger.info(
                        f"Demasiadas páginas vacías consecutivas ({consecutive_empty_pages}). Terminando categoría '{categoria} > {subcategoria}'")
                    break
                ...
                if len(products_elements) == 0 and consecutive_empty_pages == 1:
                    self.logger.info(
                        "Reintentando página actual una vez más...")
            ...
            if not has_next_page:
                self.logger.info(
                    f"No hay más páginas para '{categoria} > {subcategoria}' después de página {page_number}")
                break
        ...
        if page_number > max_pages:
            self.logger.warning(
                f"Alcanzado límite máximo de páginas ({max_pages}) para '{categoria} > {subcategoria}'")
        elif consecutive_empty_pages >= max_consecutive_empty:
            self.logger.info(
                f"Categoría terminada por páginas vacías consecutivas ({consecutive_empty_pages}) en '{categoria} > {subcategoria}'")

        self.logger.info(
            f"RESUMEN '{categoria} > {subcategoria}': {total_products_scraped} productos en {page_number} páginas")
        self.logger.info(
            f"Request #{category_index} COMPLETADO: '{categoria} > {subcategoria}' - Lista para siguiente")
        ...
        try:
            if not page.is_closed():
                await page.close()
                self.logger.debug(
                    f"Página cerrada correctamente para '{categoria} > {subcategoria}'")
        except Exception as e:
            self.logger.warning(f"Error cerrando página: {e}")

    def extract_product_data(self, product_element, categoria, subcategoria):
        try:
            name_list = product_element.xpath(
                plaza_vea.XPATH_PRODUCT_NAME).getall()
            if not name_list:
                return None
            name = name_list[0].strip()

            price_list = product_element.xpath(
                plaza_vea.XPATH_PRODUCT_PRICE).getall()
            if not price_list:
                return None
            ...
            unit_reference_list = product_element.xpath(
                plaza_vea.XPATH_PRODUCT_UNIT_REFERENCE).getall()
            unit_reference = unit_reference_list[0].strip(
            ) if unit_reference_list else ""
        ...
        try:
            ...
            for selector in selectors_to_try:
                try:
                    ...
                    self.logger.info(
                        f"Productos encontrados con selector: {selector}")
                    success = True
                    break
                except Exception as e:
            ...
            if not success:
                self.logger.warning(
                    "Ningún selector de productos funcionó, continuando sin esperar")
    ...
    async def go_to_next_page(self, page: Page, scrapy_selector):
        try:
            ...
            pagination_element = scrapy_selector.xpath(
                plaza_vea.XPATH_PAGINATION).get()
            if not pagination_element:
                self.logger.info("No se encontró elemento de paginación")
                return False

            active_page_list = scrapy_selector.xpath(
                plaza_vea.XPATH_ACTIVE_PAGE).getall()
            ...
            all_pages = scrapy_selector.xpath(
                plaza_vea.XPATH_ALL_PAGES).getall()
            all_page_numbers = [int(p) for p in all_pages if p.isdigit()]
            max_page = max(
                all_page_numbers) if all_page_numbers else current_page

            self.logger.info(
                f"Páginas disponibles: {sorted(all_page_numbers)}, Máxima: {max_page}")

            if current_page >= max_page:
                self.logger.info(
                    f"Ya estamos en la última página ({current_page}/{max_page})")
                return False

            next_page_number = current_page + 1

            # Usar XPath optimizado desde constantes
            specific_page_xpath = plaza_vea.XPATH_SPECIFIC_PAGE_BUTTON.format(
                next_page_number)
            ...
            if next_page_element:
                ...
                self.logger.info(
                    f"Haciendo clic para navegar a página {next_page_number}")
                ...
                new_active_page = new_selector.xpath(
                    plaza_vea.XPATH_ACTIVE_PAGE).getall()

                if new_active_page and int(new_active_page[0]) == next_page_number:
                    self.logger.info(
                        f"Navegación exitosa a página {new_active_page[0]}")
                    return True
                else:
                    self.logger.warning(
                        f"No se detectó cambio correcto de página. Esperado: {next_page_number}, Obtenido: {new_active_page}")
                    return False

            else:
                self.logger.info(
                    "No se encontró botón para la siguiente página")
                return False
