...
class JumboSpider(scrapy.Spider):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        if category:
            ...
            self.start_urls = jumbo.START_URLS_SUPERMARKET_1 + \
                jumbo.START_URLS_SUPERMARKET_2 + jumbo.START_URLS_ELECTRO
    ...
    async def save_products_count(self, page, page_number, response):
        try:
            ...
            cat_sub = response.url.replace(
                "https://www.jumbocolombia.com/", "").split('/')
    ...
    def take_products_fields(self, product_card: scrapy.Selector, category, sub_category):
        ...
        try:
            ...
            if len(matches) > 0:
                ...
                matches = re.findall(
                    jumbo.REGULAR_EXPRESSION_UNITS_SIMPLE, name.lower(), re.IGNORECASE)
                if len(matches) > 0:
                    value = matches[-1]
            ...
            price_text = product_card.xpath(jumbo.XPATH_GET_PRICE).getall()
            if len(price_text) == 0:
                raise ValueError(
